...
if sys.argv.__len__() < 6:
    print("실행 인자가 부족합니다(1: 도시명 / 2: 인원수 / 3: 평균나이 / 4: 보유금액 / 5: 여행시간).")
    exit(1);

# input(노드 4) -> hidden 1(노드 10) -> hidden 2(노드 10) -> hidden 3(노트 6) -> output(노드 2: donut, pie)
# 활성함수 : ReLu
# 비용함수 : 최소제곱
# 경사하강법 : Adam
# 과적합방지 : Dropout
...
if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
    ...
    curs = conn.cursor()
    strSQL = "select board_donut, board_pie, board_people, board_age, board_money, board_time from hottrace.dbo.htBoard where city=\'" + sys.argv[1] + "\'"
    curs.execute(strSQL)
